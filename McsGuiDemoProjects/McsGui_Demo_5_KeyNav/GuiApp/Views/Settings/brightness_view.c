...
static void bv_create(View_s *p_view)
{
    ...
    FooterButton_s *p_footerButton = gui_addFooterButton(p_footer, FILE_KEY_ICON_ARROW_BACK, settings_view_navigateTo);
    gui_addFooterSpacer(p_footer);
    gui_addFooterSpacer(p_footer);
    base_setFocus(p_footerButton, true);

    keynav_down(p_slider, p_footerButton);
    keynav_up(p_footerButton, p_slider);
}
