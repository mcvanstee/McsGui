...
static void ac_create(View_s *p_view)
{
    ...
    FooterButton_s *p_footerBackButton = gui_addFooterButton(p_footer, FILE_KEY_ICON_ARROW_BACK, settings_view_navigateTo);
    gui_addFooterSpacer(p_footer);
    gui_addFooterSpacer(p_footer);
    base_setFocus(p_footerBackButton, true);

    keynav_connectLeftRight(p_accentRedCB, p_accentBlueCB);
    keynav_connectLeftRight(p_accentBlueCB, p_accentGreenCB);

    keynav_connectLeftRight(p_cursorRedCB, p_cursorBlueCB);
    keynav_connectLeftRight(p_cursorBlueCB, p_cursorGreenCB);

    keynav_connectUpDown(p_accentRedCB, p_cursorRedCB);
    keynav_connectUpDown(p_accentBlueCB, p_cursorBlueCB);
    keynav_connectUpDown(p_accentGreenCB, p_cursorGreenCB);
...
static void ac_addAccentColorButtons(View_s *p_view, Pane_s *p_pane)
{
    ...
    const Color_t accentColor = theme_getAccentColor();

    if (accentColor == COLOR_ACCENT_BLUE)
    {
        radiogroup_setSelectedAtIndex(p_radioGroup, 1);
    }
    else if (accentColor == COLOR_ACCENT_GREEN)
    {
...
static void ac_addCursorColorButtons(View_s *p_view, Pane_s *p_pane)
{
    ...
    const Color_t cursorColor = theme_getCursorColor();

    if (cursorColor == COLOR_ACCENT_BLUE)
    {
        radiogroup_setSelectedAtIndex(p_radioGroup, 1);
    }
    else if (cursorColor == COLOR_ACCENT_GREEN)
    {
        radiogroup_setSelectedAtIndex(p_radioGroup, 2);
    }
    else
...
static void ac_accentColorSelectionChanged(RadioGroup_s *p_radioGroup)
{
    ...
    if (selectedIndex == 1)
    {
        ...
        accentColor = COLOR_ACCENT_BLUE;
    }
    else if (selectedIndex == 2)
    {
        accentColor = COLOR_ACCENT_GREEN;
    }
    else
    {
        accentColor = COLOR_ACCENT_RED;
    }

    theme_setAccentColor(accentColor);
}
...
static void ac_cursorColorSelectionChanged(RadioGroup_s *p_radioGroup)
{
    ...
    if (selectedIndex == 1)
    {
        cursorColor = COLOR_ACCENT_BLUE;
    }
    else if (selectedIndex == 2)
    {
        cursorColor = COLOR_ACCENT_GREEN;
    }
    else
    {
        cursorColor = COLOR_ACCENT_RED;
    }

    theme_setCursorColor(cursorColor);
}
