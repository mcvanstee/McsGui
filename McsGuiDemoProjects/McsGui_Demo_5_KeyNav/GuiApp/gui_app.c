...
static void gui_app_initExternalDisplay(void)
{
    ...
    strcpy(setup.description, "McsGui Demo 5!");
    ed_setSetup(setup);
    gui_log_debug("Log is working");

    char logMessage[64] = {0};
    snprintf(logMessage, sizeof(logMessage), "GraphicsInstr size: %d", sizeof(GraphicsInstruction_s));
    gui_log_debug(logMessage);
}
...
static void gui_app_showBootLogo(const bool showAnimation)
{
    ...
    for (; i < 24; i++)
    {
        ...
        base_setDimensions(&logo, STYLE_DISPLAY_WIDTH, STYLE_DISPLAY_HEIGHT);
