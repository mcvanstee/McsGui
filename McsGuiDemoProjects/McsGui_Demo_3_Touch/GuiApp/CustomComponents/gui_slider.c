...
#define GUI_SLIDER_HEIGHT 22
#define GUI_SLIDER_BAR_HEIGHT 5
#define GUI_SLIDER_KNOB_WIDTH 18
...
static void slider_btn_delete(BaseComponent_s *p_buttonBase);
...
void slider_init(Slider_s *p_slider)
{
    ...
    // Because the buttons are not added with the button_new function,
    // and therefore should not be deleted with the button_delete function.
    // The delete function is called because the buttons are added as children to the slider.
    // Set the delete fuction to a dummy function.
    //
    base_setOnDelete(&p_slider->minusButton, slider_btn_delete);
    base_setOnDelete(&p_slider->plusButton, slider_btn_delete);
    ...
    p_slider->colorLeft = COLOR_ACCENT_RED;
    p_slider->colorRight = COLOR_SEPARATOR;
}
