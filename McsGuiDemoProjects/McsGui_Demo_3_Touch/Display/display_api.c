...
void display_drawPixel(const uint16_t x, const uint16_t y, const uint32_t color)
{
    ...
    {
        display_hal_writePixelData(&color565, 1);
    }
}
...
{
    const uint32_t pixelsToWrite = imagePixelWidth * imagePixelHeight;

    const bool isInAddressWindow = m_displayDriver.setAddressWindow(x, y, imagePixelWidth, imagePixelHeight);

    if (isInAddressWindow)
    {
        if (DISPLAY_COLOR_BUFFER_SIZE_PIXELS >= pixelsToWrite)
        {
            display_hal_writePixelData(p_imageData, pixelsToWrite);
            display_hal_transmissionFinished();
        }
        else
        {
            uint32_t pixelsToTransfer = pixelsToWrite;
            uint32_t readIndex = 0;

            while (pixelsToTransfer > 0)
            {
                if (pixelsToTransfer >= DISPLAY_COLOR_BUFFER_SIZE_PIXELS)
                {
                    pixelsToTransfer -= DISPLAY_COLOR_BUFFER_SIZE_PIXELS;
                    display_hal_writePixelData(&p_imageData[readIndex], DISPLAY_COLOR_BUFFER_SIZE_PIXELS);
                    readIndex += DISPLAY_COLOR_BUFFER_SIZE_PIXELS;
                }
                else
                {
                    display_hal_writePixelData(&p_imageData[readIndex], pixelsToTransfer);
                    pixelsToTransfer = 0;
                }
