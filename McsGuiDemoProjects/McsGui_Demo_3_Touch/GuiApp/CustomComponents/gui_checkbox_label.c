...
{
    checkbox_label_init(p_checkboxLabel);
    base_setBmpKey(p_checkboxLabel, FILE_KEY_ICON_SWITCH);
    p_checkboxLabel->label = label;
    p_checkboxLabel->labelPadding = 60;
}

void checkbox_label_displayLabel(CheckboxLabel_s *p_checkboxLabel)
{
    Label_s label;
    label_initBmp(&label, p_checkboxLabel->label);
    base_setWidth(&label, p_checkboxLabel->labelWidth);

    if (p_checkboxLabel->labelWidth != 0)
    {
    ...
    GuiAnchor_s anchor;
    anchor_init(&anchor);
    base_addAnchor(&label, &anchor);
    anchor_setLeftAnchor(&label, p_checkboxLabel, Gui_Anchor_Left);
    anchor_setLeftMargin(&label, p_checkboxLabel->labelPadding);
    anchor_setVerticalCenter(&label, p_checkboxLabel);
    anchor_setTopMargin(&label, 1);
...
{
	p_checkboxLabel->onSelectionChanged = onSelectionChanged;
}
...
static void checkbox_label_selectionChanged(Checkbox_s *p_checkbox)
{
    ...
    CheckboxLabel_s *p_checkboxLabel = (CheckboxLabel_s*)p_checkbox;

	if (p_checkboxLabel->onSelectionChanged != NULL)
	{
		p_checkboxLabel->onSelectionChanged(p_checkboxLabel);
	}
