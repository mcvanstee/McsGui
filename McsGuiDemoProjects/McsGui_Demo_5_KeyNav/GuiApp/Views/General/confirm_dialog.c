...
static void cd_create(View_s *p_view, const file_key_e title, const file_key_e message)
{
    ...
    FooterButton_s *p_cancelButton = gui_addFooterButton(p_footer, FILE_KEY_ICON_CANCEL, m_p_cancelBtnPressed);
    gui_addFooterSpacer(p_footer);
    FooterButton_s *p_okButton = gui_addFooterButton(p_footer, FILE_KEY_ICON_CHECKMARK_LARGE, m_p_okBtnPressed);
    base_setFocus(p_okButton, true);

    keynav_right(p_cancelButton, p_okButton);
    keynav_left(p_okButton, p_cancelButton);
}
