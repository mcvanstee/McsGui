...
static void set_alarm_view_create(View_s *p_view)
{
    ...
    theme_setPaneText(p_minutesText);
    ...
    FooterButton_s *p_backButton = gui_addFooterButton(p_footer,
            FILE_KEY_ICON_ARROW_BACK, mainview_navigateTo);
    gui_addFooterSpacer(p_footer);

    FooterButton_s *p_saveButton = gui_addFooterButton(p_footer,
            FILE_KEY_ICON_CHECKMARK_LARGE, sav_saveAlarm);
    base_setFocus(p_backButton, true);

    keynav_up(p_backButton, &p_numberInput->decreaseButton);
    keynav_up(p_saveButton, &p_numberInput->increaseButton);

    keynav_down(&p_numberInput->decreaseButton, p_backButton);
    keynav_down(&p_numberInput->increaseButton, p_saveButton);
